...
type IdleDirection = `idle-${Direction}`
type RunDirection = `run-${Direction}`
type WalkDirection = `walk-${Direction}` | `walk-${Direction}-backwards`

export type BasicAnimationName = IdleDirection | RunDirection | WalkDirection

type BasicAnimationKeyframe = {
  name: BasicAnimationName
  frames: number[]
  frameRate: number
}

const basicAnimationKeyframes: BasicAnimationKeyframe[] = [
  {
    name: 'idle-down',
    frames: [0, 1, 2, 1],
    frameRate: 5,
